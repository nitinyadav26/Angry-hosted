...
public class Level1_Birds implements Screen {
    ...
    public Level1_Birds(AngryBirdsGame game) {
        this.game = game;
    ...
    public void render(float delta) {
        ...
        if (isButtonHovered(buttonWidth, buttonHeight, proceedButtonX, proceedButtonY)) {
            ...
            if (Gdx.input.isTouched()) {
                GamePlayScreen g1 = new GamePlayScreen(game);
                game.setScreen(g1);
            }
        } else {
    ...
    @Override
    public void hide() {
        game.backgroundMusic.stop(); // stop music when hiding the screen
    }
